...
function ClientHistory({ openedTiket = null, client = {}, CLIENT_HISTORY_ITEMS }) {
    const [amcticketList, setAmcamcticketList] = useState([]);
    ...
    const getClientsAmcamcticket = async () => {
        try {
            const res = await makeRequest("amctickets", {
                method: "POST",
            ...
            if (res?.success) {
                setAmcamcticketList(res.data || []);
            } else {
                setAmcamcticketList([]);
            }
        } catch (error) {
            console.error("Failed to fetch amctickets:", error);
            setAmcamcticketList([]);
        }
    };
    ...
    return (
        <div className="client-history-panel">
            ...
            <div className="histories ticket-scroll-pane px-2 space-y-2 mt-2">
                {amcticketList.filter((item) => item.amcticket_id !== openedTiket).map((item) => (
                    <article key={item.amcticket_id} className="relative rounded-sm border border-slate-200 bg-white  px-4 py-2 shadow-sm" >
                        <span className={`absolute top-2 right-2 rounded-full text-[${item.status_color}] bg-[${item.status_color}] px-2.5 py-0.5 text-[0.55rem] shadow-sm font-medium`} style={{ backgroundColor: item.status_color , color: "white" }}>
                            {item?.amcticket_status}
                        </span>
                        <div className="flex items-center  justify-center gap-3">
                            <h4 className=" font-semibold text-slate-800 w-full">
                                {item.amcticket_no && <span className="mr-2.5 text-orange-400 text-[12px]">{item.amcticket_no} <br /></span>}
