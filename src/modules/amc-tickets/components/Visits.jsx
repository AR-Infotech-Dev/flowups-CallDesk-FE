...
function Visits({ amcticket = {}, amcticket_id }) {
  const resolvedAmcamcticketId = amcticket_id || amcticket?.amcticket_id;
  ...
  const fetchVisits = async () => {
    if (!resolvedAmcamcticketId) return;

    try {
      setLoading(true);
      const res = await makeRequest("amctickets/visits", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amcticket_id: resolvedAmcamcticketId }),
      });
  ...
  const handleSchedule = async () => {
    ...
    try {
      setSaving(true);
      const res = await makeRequest("amctickets/visits/create", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amcticket_id: resolvedAmcamcticketId,
          employee_id: amcticket?.assignee || null,
  ...
  const handleMarkVisited = async (visit) => {
    if (!resolvedAmcamcticketId || !visit?.visit_id) return;

    try {
      setSaving(true);
      const res = await makeRequest("amctickets/visits/visited", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
