...
function WorkLogs({ amcticket = {}, amcticket_id, onAfterSave }) {
  const authId = localStorage.getItem("_auth_id");
  const resolvedAmcticketId = amcticket_id || amcticket?.amcticket_id;
  ...
  const canAddLog = useMemo(() => {
    ...
    return Number(amcticket?.assignee || 0) === Number(authId || 0);
  }, [summary?.can_add_log, amcticket?.assignee, authId]);

  const fetchLogs = async () => {
    if (!resolvedAmcticketId) return;

    try {
      setLoading(true);
      const res = await makeRequest("amctickets/work-logs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amcticket_id: resolvedAmcticketId }),
      });
  ...
  const handleStartWork = async () => {
    if (!canAddLog || !resolvedAmcticketId) return;
    if (activeLog) {
    ...
    try {
      setSaving(true);
      const res = await makeRequest("amctickets/work-logs/create", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amcticket_id: resolvedAmcticketId,
          work_status: "working",
        }),
      });
  ...
  const handleEndWork = async () => {
    ...
    try {
      setSaving(true);
      const res = await makeRequest("amctickets/work-logs/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
