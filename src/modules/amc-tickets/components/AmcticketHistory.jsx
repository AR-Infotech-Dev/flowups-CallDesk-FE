...
function getMessage(item) {
    if (item.action_type === "created") {
        return item.comment || "New amcticket created.";
    }

    if (item.action_type === "reassigned") {
        return (
            <>
                Amcticket assigned to{" "}
...
function labelText(field) {
    const labels = {
        amcticket_status: "Status",
        amcticket_priority: "Priority",
        due_date: "Due date",
...
// MAIN
// =====================================================
function AmcticketHistory({ amcticket_id }) {
    const [history, setHistory] = useState([]);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        const getHistory = async () => {
            if (!amcticket_id) return setHistory([]);

            try {
                setLoading(true);

                const res = await makeRequest("amctickets/history", {
                    method: "POST",
                    body: { amcticket_id },
                });
...
export default AmcticketHistory;