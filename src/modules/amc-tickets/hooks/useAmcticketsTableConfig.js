...
import { amcticketsFallbackColumns, ticketsModuleSchema } from "../data/module.schema";
import { TICKET_VISIBILITY_COLUMN } from "../utils/amctickets.utils";

export const useAmcticketsTableConfig = ({ resolvedMenuID, filterState, role_slug }) => {
  ...
  const resolvedColumns = useMemo(() => {
    const columns = buildTableColumnsFromStructure(fields, amcticketsFallbackColumns, columnOptions);
  ...
  const defaultVisibleColumnKeys = useMemo(
    () => [...amcticketsFallbackColumns.map((column) => column.key), TICKET_VISIBILITY_COLUMN.key],
    []
  );
  ...
  const resolvedFilterFields = useMemo(() => {
    const filterFields = buildFilterFieldsFromStructure(
      ...
      ticketsModuleSchema.defaultColumns.map((key) => ({
        label: amcticketsFallbackColumns.find((column) => column.key === key)?.label || key,
        value: key,
        type: "text",
      })),
      columnOptions
