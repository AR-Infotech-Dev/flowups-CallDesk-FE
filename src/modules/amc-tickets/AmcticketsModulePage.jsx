...
import AmcticketForm from "./components/AmticketForm";
import AmcticketTableRow from "./components/AmcticketTableRow";
import { ticketsModuleSchema } from "./data/module.schema";
import { useAmcticketsModule } from "./hooks/useAmcticketsModule";
import { useAmcticketsTableConfig } from "./hooks/useAmcticketsTableConfig";
import { selectAmcticketsDefaultFilters, setAmcticketsPage, setAmcticketsViewAll } from "./data/anctickets.slice";
function AmcticketsModulePage({ menu_id }) {
  ...
  const ticketDefaultFilters = useAppSelector(selectAmcticketsDefaultFilters);
  const {
    filterState,
    setSearchText,
    applyFilterPayload,
  ...
  const {
    ...
    bumpKanbanReload,
  } = useAmcticketsModule({ resolvedMenuID, filterState });

  const { sortConfig, resolvedColumns, defaultVisibleColumnKeys, resolvedFilterFields, } = useAmcticketsTableConfig({ resolvedMenuID, filterState, role_slug: roleSlug });
  ...
  const handleViewAllToggle = () => {
    if (page !== 1) {
      dispatch(setAmcticketsPage(1));
    }
    dispatch(setAmcticketsViewAll(!viewAll));
  };
  ...
  return (
    <>
      <ModulePageLayout
        ...
        controls={
          <div className="tickets-module-controls flex flex-col gap-3">
            ...
            >
              ...
              <div className="amcticket-view-controls flex items-center justify-end gap-2">
                <select
                  className="amcticket-quick-filter h-7 min-w-[140px] rounded-md border border-slate-200 bg-white px-2 text-xs font-medium text-slate-700 hover:border-slate-300 focus:ring-2 focus:ring-blue-100"
                  value={quickFilter}
                ...
                <div className="view-switch" data-view={viewMode} aria-label="Amcticket view mode">
                  {/* <button
                    type="button"
                ...
                {canViewAll ? (
                  <ActionButton
                    variant="ghost"
                    className={viewAll ? "amcticket-view-all-button active" : "amcticket-view-all-button"}
        ...
        table={viewMode === "table" ? (
          ...
          <KanbanBoard
            rows={[]}
            menuId={resolvedMenuID}
            config={amcticketsModuleSchema.kanban}
            loading={false}
            lazyLoad
      ...
      <AmcticketForm
        isOpen={isFlyoutOpen}
        onClose={closeFlyout}
        selectedAmcticket={selectedAmcticket}
        onAfterSave={handleAfterAmcticketSave}
...
export default AmcticketsModulePage;
