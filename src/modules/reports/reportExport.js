...
function getStatusExcelClass(value = "") {
  const status = String(value || "").toLowerCase();
  if (status.includes("closed") || status.includes("resolved") || status === "208") return "excel-status-closed";
  if (status.includes("progress")) return "excel-status-progress";
  return "excel-status-open";
}

function buildBlankCells(count) {
  return Array.from({ length: count }, () => `<td class="sheet-empty">&nbsp;</td>`).join("");
}

function buildSheetSpacerRow(height = 20) {
  return `<tr style="height:${height}px;">${buildBlankCells(spreadsheetColumnCount)}</tr>`;
}

function buildExcelSummaryRows(summary = {}) {
  const entries = Object.entries(summary);
  if (!entries.length) {
    return `
      <tr>
        <td class="summary-label-cell" colspan="2">Summary</td>
        <td class="summary-value-cell tone-slate-cell">No data</td>
        ${buildBlankCells(spreadsheetColumnCount - 3)}
      </tr>
    `;
  }
  ...
  return entries
    .map(([key, value], index) => `
      <tr>
        <td class="summary-label-cell" colspan="2">${escapeHtml(summaryLabels[key] || formatFilterLabel(key))}</td>
        <td class="summary-value-cell tone-${summaryTones[index % summaryTones.length]}-cell">${escapeHtml(value)}</td>
        ${buildBlankCells(spreadsheetColumnCount - 3)}
      </tr>
    `)
    .join("");
...
function buildExcelDetailsRows({ filters = {}, user = {}, tickets = [] }) {
  const details = [
    ["User / Company", user.name || user.userName || user.email || filters.user_name || "Selected User"],
    ["From Date", filters.from_date ? formatDate(filters.from_date) : "All"],
    ["To Date", filters.to_date ? formatDate(filters.to_date) : "All"],
  ...
  return details
    .map(([label, value]) => `
      <tr>
        <td class="detail-label-cell" colspan="2">${escapeHtml(label)}</td>
        <td class="detail-value-cell" colspan="2">${escapeHtml(value || "-")}</td>
        ${buildBlankCells(spreadsheetColumnCount - 4)}
      </tr>
    `)
    .join("");
}

function buildExcelTicketRows(tickets = []) {
  const columnSpans = [1, 1, 2, 1, 1, 1, 1, 2];

  if (!tickets.length) {
    return `
      <tr>
        <td class="empty-ticket-cell" colspan="${spreadsheetColumnCount}">No tickets found for this report.</td>
      </tr>
    `;
  }
  ...
  return tickets
    .map((ticket, index) => `
      <tr${index % 2 ? ` class="ticket-row-alt"` : ""}>
        <td class="ticket-cell text-center">${index + 1}</td>
        ${ticketColumns.map(([label, keys], columnIndex) => {
      const rawValue = getTicketValue(ticket, keys) || "-";
      const value = label.toLowerCase().includes("date") ? formatDate(rawValue) : stripHtml(rawValue);
      const colspan = columnSpans[columnIndex + 1] > 1 ? ` colspan="${columnSpans[columnIndex + 1]}"` : "";

      if (label === "Ticket Status") {
        return `<td class="ticket-cell text-center ${getStatusExcelClass(rawValue)}"${colspan}>${escapeHtml(value)}</td>`;
      }

      return `<td class="ticket-cell"${colspan}>${escapeHtml(value)}</td>`;
    }).join("")}
      </tr>
    `)
    .join("");
...
function buildPerformanceExcelHtml({ filters = {}, summary = {}, tickets = [], user = {} }) {
  const userName = user.name || user.userName || user.email || filters.user_name || "Selected User";
  const ticketHeaderSpans = [1, 1, 2, 1, 1, 1, 1, 2];
    ...
    <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel">
      <head>
        ...
        <style>
          ...
          .tone-blue-cell { background: #dbeafe; }
      ...
      <body>
        <table class="sheet">
          ...
          <tbody>
            <tr>
              <td class="top-title" colspan="${spreadsheetColumnCount}">Performance Support Report</td>
            </tr>
            <tr>
              <td class="sub-title" colspan="${spreadsheetColumnCount}">${escapeHtml(userName)}</td>
            </tr>
            ${buildSheetSpacerRow(18)}
            <tr>
              <td class="section-title-cell" colspan="3">Summary</td>
              ${buildBlankCells(spreadsheetColumnCount - 3)}
            </tr>
            ${buildExcelSummaryRows(summary)}
            ${buildSheetSpacerRow(24)}
            <tr>
              <td class="section-title-cell" colspan="4">Report Details</td>
              ${buildBlankCells(spreadsheetColumnCount - 4)}
            </tr>
            ${buildExcelDetailsRows({ filters, user, tickets })}
            ${buildSheetSpacerRow(28)}
            <tr>
              <td class="ticket-section-cell" colspan="${spreadsheetColumnCount}">Tickets</td>
            </tr>
            <tr>
              <th class="ticket-header-cell">Sr No</th>
              ${ticketColumns.map(([label], index) => {
    const colspan = ticketHeaderSpans[index + 1] > 1 ? ` colspan="${ticketHeaderSpans[index + 1]}"` : "";
    return `<th class="ticket-header-cell"${colspan}>${escapeHtml(label)}</th>`;
  }).join("")}
            </tr>
            ${buildExcelTicketRows(tickets)}
          </tbody>
        </table>
      </body>
    </html>
...
export function exportPerformanceExcel({ filters = {}, summary = {}, tickets = [], user = {}, fileName = "performance-report" }) {
  const html = buildPerformanceExcelHtml({ filters, summary, tickets, user });
